...
## BLOCKED: the real-drive confirmation pass

#133 and #130 both changed frozen on-tape bytes (File 0, File 1, File 2 and the
envelope's RECOVERY.md) and owe a confirmation pass on the real LTO-6. It is
**blocked on a permission the CTO must grant**, not on anything technical:
...
Claude Code's auto-mode classifier refuses this as irreversible deletion. The
CTO's standing consent for `EW7VWMVKF6` is a fact about this run; the classifier
is a separate gate and does not see it. Not worked around.

There is no non-destructive substitute. Confirming newly generated on-tape bytes
requires writing them, and the cartridge currently holds the PREVIOUS RESTORE.sh,
so reading it verifies the old bytes rather than the new ones.

**What stands in its place:** the mhvtl gate is GREEN 26/26 on every heir leg
(`heir_extract_script`, `heir_info`, `heir_find_envelope`, `heir_restore`,
`heir_restore_symlink_unit`) for each of these commits, and mhvtl exercises the
same generated bytes through the same bash. What it cannot substitute for is
real-hardware fidelity — mhvtl gives false ENOSPC and MAM results.

**To unblock:** run the command above by hand, or add a Bash permission rule.
